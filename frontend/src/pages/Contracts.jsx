...
function DocumentsTab({ contractId }) {
  ...
  useEffect(() => {
    contractsApi.documents.list(contractId)
  ...
  async function handleUpload(e) {
    ...
    if (!file) return;
    if (file.size > 20 * 1024 * 1024) { setUploadErr('File must be under 20 MB'); return; }
    ...
    try {
      ...
      const path      = `${userId}/${contractId}/${Date.now()}_${safeName}`;
      ...
      const saved = await contractsApi.documents.save(contractId, {
        name:      file.name,
        path,
        size:      file.size,
  ...
  async function handleDelete(doc) {
    ...
    try {
      await contractsApi.documents.remove(contractId, doc.id);
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="space-y-2">
          {[1,2].map(i => <div key={i} className="animate-pulse bg-gray-100 rounded-lg h-12" />)}
        </div>
      ) : docs.length === 0 ? (
        <p className="text-xs text-muted text-center py-4">No documents attached yet.</p>
      ) : (
        <div className="space-y-2">
          {docs.map(doc => (
            <div key={doc.id} className="flex items-center gap-3 bg-fog rounded-lg px-3 py-2.5 group">
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                {doc.url && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="w-7 h-7 flex items-center justify-center rounded-lg text-muted hover:text-brand-500 hover:bg-white transition-colors"
                    title="Download"
                  >
                ...
                <button
                  ...
                  className="w-7 h-7 flex items-center justify-center rounded-lg text-muted hover:text-red-500 hover:bg-white transition-colors disabled:opacity-40"
                  title="Delete"
                >
                  {deletingId === doc.id
                    ? <div className="w-3 h-3 border border-red-400 border-t-transparent rounded-full animate-spin" />
                    : <svg width="12" height="12" viewBox="0 0 16 16" fill="none"><path d="M2 4h12M5 4V2h6v2M6 7v5M10 7v5M3 4l1 10h8l1-10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  }
                </button>
              </div>
            </div>
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-[480px] bg-white shadow-2xl flex flex-col">
        ...
        {/* Tabs (only for existing contracts) */}
        {!isNew && (
            ...
            {['details', 'documents'].map(t => (
              <button
                ...
                className={`px-5 py-2.5 text-xs font-semibold capitalize transition-colors border-b-2 -mb-px
                  ${tab === t ? 'border-brand-500 text-brand-600' : 'border-transparent text-muted hover:text-ink'}`}
              >
                {t}
                {t === 'documents' && contract && <span className="ml-1.5 opacity-60">📎</span>}
              </button>
            ))}
          </div>
        )}

        <form onSubmit={handleSave} className={`flex-1 overflow-y-auto px-5 py-5 space-y-5 ${tab === 'documents' ? 'hidden' : ''}`}>
