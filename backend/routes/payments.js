...
// Map Stripe price IDs → internal tier names (set these in .env after creating products)
...
function stripeStatusToInternal(stripeStatus) {
  switch (stripeStatus) {
    ...
    default:                    return 'trial'; // trialing, incomplete
  }
}
...
router.post('/create-checkout', requireAuth, async (req, res, next) => {
  try {
    ...
    // Get client record for email + existing stripe customer id
...
// Redirects the customer to their Stripe billing portal to manage subscription
...
// server.js mounts this at POST /webhook/stripe with express.raw() body parser
export async function stripeWebhookHandler(req, res) {
  ...
  try {
    ...
    // Still return 200 to prevent Stripe retrying — log and alert separately
  }
